# test_model.py — Forward pass and loss smoke test for the baseline CNN.
...
def main():
    ...
    report_path = "outputs/metrics/model_summary.txt"
    ...
    # Shape: [batch_size=4, channels=1, mels=128, time=251]
    batch_size = 4
    ...
    # 3. Run forward pass
    model.train() # Set to train mode so gradients can be computed later
    logits = model(x)
    print(f"  Output shape: {logits.shape}")
    print(f"  Output dtype: {logits.dtype}")
    
    # 4 & 5. Verify output constraints
    ...
    # Create dummy labels (0=bonafide, 1=spoof) matching batch size
    # Shape: [4] -> needs to be [4, 1] for BCEWithLogitsLoss to match logits shape
