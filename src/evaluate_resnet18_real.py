...
from src.model import AudioResNet18
...
def main():
    ...
    model_path = f"models/resnet18_real/best_resnet18_real.pth"
    history_file = f"outputs/metrics/resnet18_real_training_history.csv"
    ...
    df_preds.to_csv(f"outputs/predictions/resnet18_real_dev_predictions.csv", index=False)
    
    # 2. Confusion Matrix & Core Metrics (Threshold = 0.5)
    os.makedirs(f"outputs/plots/resnet18_real", exist_ok=True)
    plot_confusion_matrix(targets, preds, f"outputs/plots/resnet18_real/dev_confusion_matrix.png")
    ...
    pd.DataFrame(ta_data).to_csv(f"outputs/metrics/resnet18_real_dev_threshold_analysis.csv", index=False)
    
    with open(f"outputs/metrics/resnet18_real_selected_threshold.json", "w") as f:
        json.dump({"selected_threshold": best_thresh, "selection_metric": "F1", "selection_partition": "development"}, f, indent=4)
        
    plot_confusion_matrix(targets, (probs >= best_thresh).astype(float), f"outputs/plots/resnet18_real/dev_confusion_matrix_selected_threshold.png")
    ...
    with open(f"outputs/metrics/resnet18_real_dev_metrics.json", "w") as f:
    ...
    pd.DataFrame(attack_data).to_csv(f"outputs/metrics/resnet18_real_dev_per_attack.csv", index=False)
    ...
    pd.DataFrame(speaker_data).to_csv(f"outputs/metrics/resnet18_real_dev_per_speaker.csv", index=False)
    
    # 6. Plotting
    if os.path.exists(history_file):
        plot_training_curves(history_file, f"outputs/plots/resnet18_real")
        do_overfitting_analysis(history_file, f"outputs/metrics/resnet18_real_overfitting_analysis.txt")
        
    print(f"Evaluation complete for ResNet18.")
